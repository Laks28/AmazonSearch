...
public class ProductListPage extends Page{
    ...
    @FindBy(xpath = ObjectRepo.ProductListPage.PRODUCT_KW)
    @CacheLookup
    ...
    public boolean verifyProductListIsSortedByPrice(boolean ascending) throws NotImplementedException {
        ...
        syncHelper.waitForElementToBeVisible(this.driver, ObjectRepo.ProductListPage.PRODUCT_SORTED_LIST_LABEL);

        if (!ascending){
            // Which means descending => High To Low
            productSortedListString = String.format(ObjectRepo.ProductListPage.PRODUCT_SORTED_LIST_STRING, ObjectRepo
    ...
    private Select getSortByDropDown(){
        return new Select(driver.findElement(By.xpath(ObjectRepo.ProductListPage.SORT_BY)));
    }

    private WebElement getProductSortedListLabel(){
        return driver.findElement(By.xpath(ObjectRepo.ProductListPage.PRODUCT_SORTED_LIST_LABEL));
    }

    private WebElement getProductElementDivAt(int index){
        return driver.findElement(By.xpath(String.format(ObjectRepo.ProductListPage
